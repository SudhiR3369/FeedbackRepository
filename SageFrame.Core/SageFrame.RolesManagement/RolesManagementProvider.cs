...
namespace SageFrame.RolesManagement
{
    ...
    public class RolesManagementProvider
    {
        ...
        /// <summary>
        /// Connects to database and returns all the portal role list base on upset and limit.
        /// </summary>
        /// <param name="PortalID">Portal ID.</param>
        /// <param name="Username">User's name.</param>
        ...
        public List<RolesManagementInfo> GetAllRoleList(int PortalID, string Username, int PageNumber, int PageSize)
        {
            try
            {
                ...
                ParamCollInput.Add(new KeyValuePair<string, object>("@PortalID", PortalID));
                ParamCollInput.Add(new KeyValuePair<string, object>("@UserName", Username));
                ParamCollInput.Add(new KeyValuePair<string, object>("@pageNumber", PageNumber));
                ParamCollInput.Add(new KeyValuePair<string, object>("@pageSize", PageSize));
                
                return SQLH.ExecuteAsList<RolesManagementInfo>("[dbo].[usp_Article_Role_PortalRoleList]", ParamCollInput);
            }
            catch (Exception ex)
            {
        ...
        public void UpdateUserRole(Guid RoleID, bool RoleStatus)
        {
            try
            {
                ...
                param.Add(new KeyValuePair<string, object>("@RoleStatus", RoleStatus));

                handler.ExecuteNonQuery("usp_Article_RoleManagement_UpdateRoleStatus", param);
            }
            catch (Exception ex)
            {
                throw ex;
