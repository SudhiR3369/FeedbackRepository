...
namespace SageFrame.Utilities
{
    ...
    public class Config
    {
        ...
        /// <summary>
        /// Update application version shown key.
        /// </summary>
        /// <param name="version">New version.</param>
        public static void UpdateSageFrameVersionLoad(string isShown)
        {
            //save the current config files           
            XmlDocument xmlConfig = LoadVersionConfig();
            string key = "SageFrameVersionLoad";
            XmlNode xmlConnection = xmlConfig.SelectSingleNode("appSettings/add[@key='" + key + "']");
            UpdateAttribute(xmlConnection, "value", isShown);
            SaveVersionConfig(xmlConfig);
        }
        /// <summary>
        ...
        /// </summary>
        /// <returns>"true" if instaled.</returns>
        public static string GetVersionConfigSettingValue(string key)
        {
            string _sageValue = GetSetting(key).ToString();
            return _sageValue;
        }
    }
}
