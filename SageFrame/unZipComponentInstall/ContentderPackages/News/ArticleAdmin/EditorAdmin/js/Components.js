...
,    "date": {
        ...
        "afterdrop": function ($appendedParent, $appendLayer, dropped) {
            if (dropped || CacheArticle.Article==null) {
                $appendLayer.find('.article-date-wrap').find('.editor-para').text(ArticleSetting.ChangeDateFormat(new Date()));
            }
...
,    "Title": {
        ...
        "afterdrop": function ($appendedParent, $appendLayer, dropped) {
            ...
            var val = $('#txtArticleTitle').val();
            if (dropped && val!='')
                $appendLayer.find('.article-title').text(val);
        },
        "settingDOMs": {
            "tabs": {
                "Basic": {
...
,    "author": {
        ...
        "afterdrop": function ($appendedParent, $appendLayer, dropped) {
            if (dropped) {
                if (CacheArticle != null && CacheArticle.Article != null) {
                    if (CacheArticle.Article.IsCustomReport) {
                        {
                            $appendLayer.find('.editor-para').text(CacheArticle.Article.CustomReport);
                            $appendLayer.find('.author-link').attr('href', '#');
                        }
                    } else {
                        if (CacheArticle.Authors != null && CacheArticle.Authors.length > 0) {
                            var author = CacheArticle.Authors[0];
                            $appendLayer.find('.editor-para').text(ArticleSetting.ChangeAuthorNameFormat(author.Author));
                            if (author.image != '')
                                $appendLayer.find('.author-image img').attr('src', '/Modules/Admin/UserManagement/UserPic/' + author.image);
                        }
                    }
                }
            }
        ...
        "remove": function ($viewDom) {
            $viewDom.find('.author-link').attr('href', 'javascript:void(0)');
        }
...
,    "category": {
        ...
        "afterdrop": function ($appendedParent, $appendLayer, dropped) {   
            ...
            if (categories!='')
              $appendLayer.find('.article-category').text(categories);
        },
        "settingDOMs": {
            "tabs": {
                "Basic": {
...
,    "image": {
        ...
        "hidden": false,
        "collection": true,
        "defaultdata": DOMCreate('div', '<a><img class="template-image" src="' + webbuildermodulepath + '/img/def5.jpg"  class="editor-image sfCol_100" /></a><span class="image-caption-wrap position-absolute editor-com-innerSpacing-top-25 editor-com-innerSpacing-right-25 editor-com-innerSpacing-left-25 editor-com-innerSpacing-bottom-25" style="display:block; width: 100%; bottom: 0px; left: 0px; background-color: rgba(16, 17, 17, 0.19);" data-pos="in-bottom"><span class="caption-text" style="color: rgb(242, 242, 242);" contenteditable="true">Image Caption Here</span></span>' + EasyLibrary.ReadDOM("imageoption"), 'editor-component image sfCol_100'),
        "afterdrop": function ($appendedParent, $appendLayer, dropped) {
            $('.change-image-settings').off().on('click', function () {
                var $this = $(this);
                var $parent = $this.parent().parent();
                var $image = $parent.find('img');
                var lstImages = $('#divArticleAssets').find('.news-images');
                if (lstImages.length > 0) {
                    var articleImages = '<div class="fullpage-popup-search">';
                    articleImages += '<label>Choose from Media Library</label>';
                    articleImages += '<span class="value"><span id="btnBrowseMediaMgmt" class="sfBtn smlbtn-primary ">Browse</span></span></div>';
                    articleImages += '<div class="field-row clearfix">';
                    articleImages += '<div class="news-images-browse ">';
                    articleImages += lstImages.parent().html();
                    articleImages += '</div></div>';
                    var options = {
                        title: 'Change Article Image',
                        openButton: '',
                        openInCall: true,
                        width: "90%",
                        height: "90%",
                        contentHtml: articleImages,
                        afterAppend: function ($wrapper) {

                            $wrapper.find('.setDefaultImage').show().text('Use This').attr('title','');
                            $wrapper.find('#btnBrowseMediaMgmt').off('click').on('click', function () {
                                $('#btnBrowseMediaMgmt').SageMedia({
                            ...
                            $wrapper.find('.setDefaultImage').off().on('click', function () {
                                var thatImg = $(this).prev('img');
                                $image.attr('src', thatImg.attr('data-url'));
                                $wrapper.find('.fullpage-close-model').trigger('click');
                            });
                        }
                    };
                    $('#simplePopUpContainer').makeSimpleModel(options);

                } else {

                    if ($('body').find('#btnBrowseMediaMgmt').length == 0)
                        $('body').append('<span id="btnBrowseMediaMgmt" style="display:none" ></span>')
            ...
            if (dropped)
                $appendLayer.find('.change-image-settings').trigger('click');
        },
        "settingDOMs": {
            "tabs": {
                "Basic": {
                    ...
                    "onload": function ($this) {
                        ...
                        if (typeof imgLink.attr('href') != 'undefined' && imgLink.attr('href') != '')
                            $('#imgSourceURL').val(imgLink.attr('href'));

                        $('#imgSourceURL').off().on('change', function () {
                            var val = $(this).val();
                            if (val != '') {
                                imgLink.attr('href', val);
...
,    "Article Source": {
        ...
        "afterdrop": function ($appendedParent, $appendLayer, dropped) {
            if (dropped) {
                if (CacheArticle!=null&& CacheArticle.Article != null) {
                    if (CacheArticle.Article.NewsSource != '')
                        $appendLayer.find('.article-source').text(CacheArticle.Article.NewsSource);
                }
            }
        },
        "settingDOMs": {
...
,    "Text editor": {
        ...
        "afterdrop": function ($appendedParent, $appendLayer, dropped) {
           ...
           // $parent.find('.text-deitor-toolsbar').show();
            ...
            $('#btnItalic,#btnbold,#btnUnderLine,#Lists,#btnformatblock,#btnLink,#textCases').show();
        },
        "settingDOMs": {
            "tabs": {
                "Spacing": {
