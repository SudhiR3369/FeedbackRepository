...
function OptionEvent($elem) {
    ...
    $carrieroption.off().on('click', function (event) {
        var siteBody= $('.edit-area.site-body');
        var siteBottomOffset = siteBody.offset().top + siteBody.outerHeight(true);
        var $thisOption = $(this);
              if (siteBottomOffset - $thisOption.offset().top < 100)
            $thisOption.addClass('up-option');
        else
            $thisOption.removeClass('up-option');
        $('.carrier-open-option.active-options').removeClass('active-options');
        $thisOption.addClass('active-options');
        $('.carries-options').not('.hide-element').addClass('hide-element');
        $thisOption.next().removeClass('hide-element');
        ...
        if ($parent.not('.holder').hasClass('editor-col')) {
            ...
            function CheckNoRow() {
                if (IsRowRequired) {
                    ...
                    if ($('.template-comp-wrapper').find('.editor-component').length === 0) {
                        $('.template-comp-wrapper').find('.editor-col').append('<div class="column-data-empty"><h4>This is Column</h4><p>You can Drag and drop Components here</p></div>')
                    }
...
(function ($) {
    $.WebBuilder = function (p,$BuiderContainer) {
        p = $.extend({
            modulePath: '',
            DataObj: '',
            userModuleID: '',
        ...
        var WebManagement = {
            ...
            Init: function () {
                ...
                IsRowRequired = p.isRowRequired;
                if (!p.isRowRequired) {
                    component["row"].hidden = true;
                }
                //view temp details component
                if (p.isDetailsTemp) {
                    component["read more"].hidden = true;
                    component["News Advs"].hidden = false;
                    component["text"].hidden = true;
                    component["Text editor"].hidden = false;
                    
                } else {
                    component["read more"].hidden = false;
                    component["News Advs"].hidden = true;
                    component["text"].hidden = false;
                    component["Text editor"].hidden = true;
                } 
                //
                //if (WebManagement.config.isDeveloperMode.toLowerCase() == "true")
                //{
                //    console.log(extendedComps);
                ...
                //        count++;
                //        WebManagement.UpdateComponent(name, componentValue, count);
                //    }
                //});
            ...
            GetOnlineComponents: function (async, offset, limit, searchText) {
                $.ajax({
                    ...
                    dataType: 'json',
                    crossDomain: true,
                    url: SageFrameHostURL + '/Modules/WebBuilder/services/WebService.asmx/GetOnlineComponents',
            ...
            GetOnlineSites: function (async, siteOffset, siteLimit, searchText) {
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    crossDomain: true,
                    url: SageFrameHostURL + '/Modules/WebBuilder/services/WebService.asmx/GetOnlineSites',
                    success: function (data) {
            ...
            CreateSidebar: function () {
                ...
                for (var i = 0; i < componentListLen; i++) {
                    ...
                    if (typeof ($compo.afterdrop) !== 'undefined') {
                        $compo.afterdrop($('.site-body'), $('.site-body'));
                    }
                    if (!$compo['hidden']) {
            ...
            InitEvents: function () {
                ...
                PagelinkStop();
                var previewPage = SageFrameHostURL + '/' + WebManagement.config.tempPageName.replace(/ /g, '-') + WebManagement.config.pageExtension;
            ...
            AdjustTopSitckyPadding: function () {
                if ($BuiderContainer.find('.edit-area').hasClass('site-header-sticky') || $('.edit-area').hasClass('site-header-fixed')) {
            ...
            LoadDetail: function () {
                ...
                ActiveMenu();
                var data = '';
                if ($BuiderContainer.find('.editor-componentWrapper').length > 0) {
                    var htmlLength = $BuiderContainer.find('.editor-componentWrapper > div').length;
                    if (htmlLength > 0) {
                        ...
                        if (p.isRowRequired) {
                            $BuiderContainer.find('.editor-componentWrapper').append(noElement);
                        }
                        else {
                            $BuiderContainer.find('.edit-area').html($('#divHiddenColDom').html());
                            $BuiderContainer.find('.editor-col').each(function () {
                                BindColumnEvents($(this));
                            });
                        }
                    }
                    ...
                    if (p.isRowRequired) {
                        $BuiderContainer.find('.edit-area').append(DOMCreate('div', '', 'editor-componentWrapper clearfix'));
                        $BuiderContainer.find('.editor-componentWrapper').append(noElement);
                    } else {
                        if ($BuiderContainer.find('.edit-area').find('.template-comp-wrapper').length === 0) {
                            $BuiderContainer.find('.edit-area').html($('#divHiddenColDom').html());
                            $BuiderContainer.find('.editor-col').each(function () {
                                BindColumnEvents($(this));
                            });
