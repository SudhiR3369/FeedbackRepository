...
public partial class Controls_TopStickyBar : BaseAdministrationUserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        hypEdit.NavigateUrl = GetEditURL();
        ...
        if (isDashboardAccessible)
        {
            ...
            cpanel.Visible = true;
            GetNotInstallModules();
            htpTemplates.NavigateUrl = GetHostURL() + "/Admin/Templates.aspx";
            htpTemplates.Visible = true;
        }
        else
        {
    ...
    private string GetEditURL()
    {
        ...
        var pattern = new Regex("(\\w+|(\\w+-\\w+))" + Extension);//actual regex is (\w+|(\w+-\w+)) 0r (\w+-|\w+)\w+
        string navigateURL = string.Empty;
        foreach (Match m in pattern.Matches(url))
        {
            navigateURL = m.Groups[0].Value;
        }
        if (navigateURL == string.Empty || navigateURL.ToLower() == "default")
        {
            SageFrameConfig sfConfig = new SageFrameConfig();
            navigateURL = sfConfig.GetSettingsByKey(SageFrameSettingKeys.PortalDefaultPage);
        }
        navigateURL = navigateURL.Replace(Extension, "");
        navigateURL = GetHostURL() + "/Webbuilder/" + navigateURL;
        return navigateURL;
    }
