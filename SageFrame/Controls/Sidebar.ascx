    ...
    var SidebarMgr = {
        ...
        init: function () {
            if (SidebarMgr.config.SidebarMode == 0) {
                ...
                // $('ul.menu').initMenu();
                SidebarMgr.NoHighlightSelected();
            ...
            $('body').find('.dash-wrapper').addClass('defaultMenu');
            if (SidebarMgr.config.ShowSideBar == "1") {
                $('div.togglePart').on("click", function () {
                    $(this).toggleClass("growMenu");
                    $('div.sfSidebar').toggleClass("sfSidebarhide");
                    $('.dash-wrapper').toggleClass("shrinkMenu");
                    SidebarMgr.UpdateSidebarMode();
                });
                var open = $('div.sfSidebar').attr('data-mode');
                if (open == 'close') {
                    $('div.togglePart').trigger('click');
                }
                $('.parent').on('click', function () {
                    var $this = $(this);
                    if ($this.hasClass('activeParentClass')) {
                        $('.parent').removeClass('activeParentClass');
                        $('.parent').find('.acitem').hide();
                    }
                    else {
                        $('.parent').removeClass('activeParentClass');
                        $('.parent').find('.acitem').hide();
                        $this.addClass('activeParentClass');
                        $this.find('.acitem').slideDown('400');
                    }
                    //$(this).find('.acitem').slideToggle(400);
                    //$(this).toggleClass('selectedNav');

                });
                //$(this).find('.acitem').slideToggle(400);
                //$(this).toggleClass('selectedNav');
                $('div.togglePartNo').on("click", function () {
        ...
        HighlightSelected: function () {
            ...
            $("ul.acitem.active").parent().addClass('selectedNav');
        },
        NoHighlightSelected: function () {
            var sidebar = $('#sidebar ul li');
            $.each(sidebar, function (index, item) {
                if ($(this).hasClass("parent")) {
                    var submenu = $(this).find("ul li");
                    $.each(submenu, function () {
                        var hreflink = $(this).find("a").attr("href");
                        if (location.href.toLowerCase().indexOf(hreflink.toLowerCase()) > -1) {
                            $(this).parent("ul.acitem").addClass("active");
                            $(this).parent("ul.acitem").prev("a").addClass("active");
                            $(this).parent("ul.acitem").parents('li.Grandparent').css("display", "block").addClass("active");
                        }
                    });
