...
$(function () {
    SageFrame = {
        config: {
            ...
            url: "",
            ajaxCallMode: 0,
            baseURL: SageFrameAppPath + "/Sage_Services/SageFrameGlobalWebService.asmx/"
        },
        vars: {
...
$(function () {
    ...
    $('.themeSettingPart').on("click", function () {
        $(this).toggleClass("thisSelected");
        $('#templateChangeWrapper').slideToggle(400);
    });

    $('li.logout').off().on("click", function () {
        $('.myProfileDrop').slideToggle(400);
    });
    //topbar text hiding
    if ($(window).width() <= 768) {
        $('#hypHome').html('');
    ...
    $('.WebBuilderWrapper').parent('.ajax__tab_panel').css('padding', '0px');

    // $('.ajax__tab_body').siblings('.ajax__tab_header').css('display', 'none');
    // $('.WebBuilderWrapper').parent('.ajax__tab_panel').siblings('.ajax__tab_header').css('display', 'none');



    //dashboard boxex hide show

    // $('.window-box').find('.fa-minus').on('click', function(){
    //     $('.window-box').find('.box-content ').slideToggle(400);
    // })

    // $('.window-box').find('.fa-times').on('click', function(){
    //     $('.window-box').fadeOut(400);
    // })
    var userColors = $('.hdnColorPallet').val();
    if (typeof (userColors) !== 'undefined') {
        var colors = userColors.split(',');
        var userFirstColor = '#000';
        var userSecondColor = '#000';
        userFirstColor = colors[0];
        userSecondColor = colors[1];


        $("#UserFirstColor").ColorPicker({
            color: "#000",
            ...
            onBeforeShow: function () {
                $('#UserFirstColor').ColorPickerSetColor(userFirstColor);
            },
            onChange: function (hsb, hex, rgb) {
                $('#UserFirstColor').css('backgroundColor', '#' + hex);
                $("#UserFirstColor").attr('data-value', '#' + hex);
                UpdateColor();
            }
        });

        $("#UserSecondColor").ColorPicker({
            color: "#000",
            onShow: function (colpkr) {
                $(colpkr).fadeIn(500);
                return false;
            ...
            onBeforeShow: function () {
                $('#UserSecondColor').ColorPickerSetColor(userSecondColor);
                //$('#divBodyTextColor').ColorPickerSetColor("#000");
            },
            onChange: function (hsb, hex, rgb) {
                $('#UserSecondColor').css('backgroundColor', '#' + hex);
                $("#UserSecondColor").attr('data-value', '#' + hex);
                UpdateColor();
            }
        });
        function UpdateColor() {
            $('.hdnColorPallet').val($("#UserFirstColor").attr('data-value') + ',' + $("#UserSecondColor").attr('data-value') + ',user' + ", ");
        }
        function CalculatesidebarHeight() {
            var pageHeight = $('.sfMaincontent').height() - Math.abs($('#divAdminControlPanel').height()) + 5;
