(function ($) {
    ...
    $.createNotification = function (p) {
        ...
        var AdminNotification = {
            ...
            init: function () {
                AdminNotification.LoadAdminNotification(1, 10, 0);
                $('#txtSearchFaq').keypress(function (e) {
                    if (e.which == 13) {
                        ...
                        if (searchText == '' || searchText == '-- Search Your Question --') {
                            ...
                            $('.sfEmptySearch').hide();
                            AdminNotification.GetSearchList(searchText);
                            $('#txtSearchFaq').val('');
                        }
                    }
                });
                ...
                $('#btnHelpful').live('click', function () {
                    ...
                    AdminNotification.SubmitFAQViewOption(optionID, FAqId);
                    $(this).parent().hide();
                ...
                $('#btnSendReason').live('click', function () {
                    ...
                    AdminNotification.SendReason(reason, userEmail, QId);
            ...
            ajaxSuccess: function (data) {
                switch (AdminNotification.config.ajaxCallMode) {
                    case 0:
                        break;
                    case 1:
                        AdminNotification.BindAdminNotification(data);
                        break;
                    case 2:
                        break;
                }
            ...
            GetSearchList: function (searchword) {
                resultCount = 1;
                AdminNotification.config.method = "GetSearchList";
                AdminNotification.config.url = AdminNotification.config.baseURL + AdminNotification.config.method;
                AdminNotification.config.data = JSON2.stringify({ PortalID: AdminNotification.config.PortalID, UserModuleID: AdminNotification.config.UserModuleID, CultureName: AdminNotification.config.CultureName, SearchWord: searchword });
                AdminNotification.config.ajaxCallMode = 1;
                AdminNotification.ajaxCall(AdminNotification.config);
            },

            LoadAdminNotification: function (offset, limit, current) {
                currentPage = current;
                AdminNotification.config.method = "GetAdminNotification";
                AdminNotification.config.url = AdminNotification.config.baseURL + AdminNotification.config.method;
                AdminNotification.config.data = JSON2.stringify({ PortalID: parseInt(AdminNotification.config.PortalID), UserModuleID: parseInt(AdminNotification.config.UserModuleID), CultureName: AdminNotification.config.CultureName, Offset: offset, limit: limit });
                AdminNotification.config.ajaxCallMode = 1;
                AdminNotification.ajaxCall(AdminNotification.config);
            ...
            BindAdminNotification: function (data) {
                $("#ulAdminNotification").html('');
                $('.sfEmptyresult').hide();
                var Html = "";
                var order = 1;
                var categoryCount = 1;
                ...
                $.each(data.d, function (index, value) {
                    ...
                    if (temp == value.CategoryID) {
                        Html += '<li id="' + value.FAQId + '" class="sfAdminNotification">';
                ...
                $("#ulAdminNotification").append(Html);
                $('.sfFieldItems').hide();
                $.each($('.dvCategory'), function () {
                    var me = $(this);
                    var liCount = me.find('.dvCategoryList').find('.sfAdminNotification').length;
                    liCount = '[' + liCount + ']';
                ...
                if (rowTotal > 10) {
                    if (arrItemListType.length > 0) {
                        $("#Pagination").pagination(rowTotal, {
                            ...
                            callfunction: true,
                            function_name: { name: AdminNotification.LoadAdminNotification, limit: 10 },
                            prev_text: ".",
                            next_text: ".",
            ...
            ajaxCall: function (config) {
                $.ajax({
                    type: AdminNotification.config.type,
                    contentType: AdminNotification.config.contentType,
                    async: AdminNotification.config.async,
                    cache: AdminNotification.config.cache,
                    url: AdminNotification.config.url,
                    data: AdminNotification.config.data,
                    dataType: AdminNotification.config.dataType,
                    success: AdminNotification.ajaxSuccess,
                    error: AdminNotification.ajaxFailure
                });
            }
        };
    ...
    $.fn.SageAdminNotification = function (p) {
