        ...
        var PageTreeView = {
            ...
            ajaxSuccess: function (data) {
                switch (PageTreeView.config.ajaxCallMode) {
                    case 0:
                        SageFrame.messaging.show('Set as start page successful.', "success");
                        break;
                    case 1:
            ...
            BindEvents: function () {
                ...
                $('#navContainer').on('click', '.deletePage', function () {
                    ...
                    if (pageName == "Under Construction") {
                        SageAlertDialog('Under Construction page cannot be deleted.');
                    }
                    else if ($me.parents('li').find('li.required').length > 0 || $me.parent("li").hasClass("required")) {
                        SageAlertDialog('Unable to  delete. Before delete change startup page.');
                    }
                    else {
                        SageConfirmDialog('Are you sure you want to delete this page ?').done(function () {
            ...
            BindPages: function (data) {
                ...
                var itemPath = "";
                var html = "";
                html += '<h3><i class="fa fa-files-o"></i>Pages</h3>';
                html += '<div class="sfSearch">';
                html += '<input type="text" id="txtSearch" class="sfInputbox" style="width: 20px;"/><span class="icon-search sfSearchPages"></span></div>';
                        ...
                        bindings: {
                            ...
                            'edit': function (t) {
                                if ($(t).parents('li').find('li.required').length > 0 || $(t).parent("li").hasClass("required")) {
                                    SageAlertDialog('Unable to edit a Portal Start Up Page');
                                }
                                else {
                                    PageTreeView.ClearControls();
                                    PageTreeView.PageShow(1);
                            ...
                            'startpage': function (t) {
                                if ($('#rdbAdmin').prop("checked")) {
                                    SageAlertDialog('Admin page cannot be start up page.');
                                }
                                else {
                            ...
                            'remove': function (t) {
                                if ($(t).text() == "Under Construction") {
                                    SageAlertDialog('Under Construction page is not deleted.');
                                }
                ...
                var upload = new AjaxUpload($('#flIcon'), {
                    ...
                    onSubmit: function (file, ext) {
                        ...
                        if (ext == "png" || ext == "jpg" || ext == "gif" || ext == "bmp" || ext == "JPEG" || ext == "jpeg" || ext == "ico") {
                            ...
                            return AlertDialog(this, 'Invalid image extension', "Not a valid image file");
                            return false;
                        }
                    },
                    onComplete: function (file, response) {
                        if (response == "LargeImagePixel") {
                            return AlertDialog(this, 'Large image pixel', "The image size is too large in pixel");
                        }
                        if (response == "large") {
                            return AlertDialog(this, 'Large image size', "The image size is too large. Should be less than 1mb");
                        }
