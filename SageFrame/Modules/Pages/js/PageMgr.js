        ...
        var SagePages = {
            ...
            BindEvents: function () {
                var v = $("#form1").validate({
                    ...
                    messages: {
                        txtPageName: "<br/>This field is required.",
                        txtTitle: "<br/>This field is required."
                    }
                });
            ...
            IconUploaderAdmin: function () {
                ...
                var upload = new AjaxUpload($('#flIcon'), {
                    ...
                    onComplete: function (file, response) {
                        ...
                        if (response == "large") {
                            SageAlertDialog('The image size is too large. Should be less than 1mb', 'Large image  size');
                        }
            ...
            AddUpdatePage: function () {
                ...
                if (pageID > 0) {
                    Mode = "E";
                    SagePages.config.pageUpdated = true;
                    SagePages.config.pageAdded = false;

                } else {
                    Mode = "A";
                    SagePages.config.pageUpdated = false;
                    SagePages.config.pageAdded = true;
                }
                var UpdateLabel = '';
            ...
            IconUploader: function () {
                ...
                //        }
                //        else {
                //            SageAlertDialog("Not a valid image file", 'Invalid image  extension');
                //            return false;
                //        }
                //    },
                //    onComplete: function (file, response) {
                ...
                $('#flIcon').on('change', function () {
                    LogoUploader($(this));
                });
                function LogoUploader($this) {
                    var formData = new FormData();
                    formData.append('pageIcon', $this[0].files[0]);
                    $.ajax({
                        type: 'post',
                        url: Path + 'UploadHandler.ashx?userModuleId=' + p.UserModuleID + '&portalID=' + SageFramePortalID + '&userName=' + SageFrameUserName + '&secureToken=' + SageFrameSecureToken,
                        data: formData,
                        success: function (response) {
                            ...
                            if (res != null && res.length > 0) {
                                //AddLogoImage(res);
                                if (res == "LargeImagePixel") {
                                    SageAlertDialog('The image size is too large in pixel', 'Large image  pixel');
                                }
                                if (res == "large") {
                                    SageAlertDialog('The image size is too large. Should be less than 1mb', 'Large image size');
                                }
                                res = res.replace('filePath=', '');
                                var pageimage = res.split(" ").join("_");
                                var filePath = p.HostURL + "/PageImages/" + res;
                                //var filePath = p.HostURL + "/PageImages/" + pageimage;
                                //var filePath = p.HostURL + "/" + pageimage;
                                $('span.filename').text(pageimage);
                                var html = '<img title="' + pageimage + '" src="' + filePath + '" /><span class="deleteIcon"><label class="sfBtn icon-close"></label></span>';
                                $('div.cssClassUploadFiles').html(html);
                            }
                        ...
                        contentType: false,
                        error: function () {
                            alert("Oops !! something went wrong", "alert");
            ...
            RefreshPage: function (data) {
                ...
                if (SagePages.config.pageAdded || SagePages.config.pageUpdated) {
                    $('#categoryTree').html('');
            ...
            PublishPage: function () {
                if ($('#categoryTree').find('li.active').hasClass('required')) {
                    SageAlertDialog('Unable to  UnPublish  a Portal Start Up Page.');
                }
                else {
                    var pageId;
