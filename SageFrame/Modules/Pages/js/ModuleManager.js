    ...
    $.createModuleManager = function (p) {
        ...
        var ModuleManager = {
            ...
            Addplaceholder: function () {
                $('#divLayoutWireframe').find('div').each(function () {
                    ...
                    if ($this.find('div').length == 0) {
                        ...
                        //    <div class="sfPosition">contact</div>
                        //    <div class="sfblocks ui-sortable ui-droppable"></div>
                        //</div>
                        $this.html(innerHTML);
                    }
                });
            ...
            UpdateModuleOrder: function () {
                var panes = $('#divLayoutWireframe div.sfblocks');
                var ModuleOrderObj = [];
                var param = {
                    lstPageModules: [],
                    portalID: parseInt(SageFramePortalID),
                ...
                $.ajax({
                    ...
                    complete: function () { FormFieldComplete(); },
                    error: ModuleManager.ajaxFailure
                });
            },
                ...
                $('i.sfManageControl').off().on('click', function () {
                    ...
                    var dialogOptions = {
                        ...
                        close: function (event, ui) {
                            //$(this).dialog('close');
                            $('.ui-icon.ui-icon-extlink').remove();
                            $('.ui-icon.ui-icon-minus').remove();
                        }
                    };
                    ...
                    $('div.ui-dialog').css({ "z-index": "3000", 'top': '0px' });
                    $('div.ui-widget-overlay').css("z-index", "2999");
                    $('#divFrame').css({ 'width': screenwidth });
                    $('#divFrame').css({ 'height': screenHeight });
                    return false;
                });
            },
