...
function runTabifier(code, type, bindID) {
    //var code = document.getElementById('i_code').value;
    //var type = document.getElementById('o_type');
    // var code = document.getElementsByClassName("DOMCollector").value;
    //    var type = "HTML";
    // type = type.options[type.selectedIndex].value;
    if ('HTML' == type) cleanHTML(code, bindID);
...
function finishTabifier(code, bindID) {
    code = code.replace(/\n\s*\n/g, '\n');  //blank lines
    code = code.replace(/^[\s\n]*/, ''); //leading space
    code = code.replace(/[\s\n]*$/, ''); //trailing space
    if (typeof (bindID) != "undefined" && typeof (bindID.val()) != "undefined") {
        bindID.val(code);
    }
    level = 0;
    hideProgress();
...
function cleanHTML(code, bindID) {
    var i = 0;
    function cleanAsync() {
        var iStart = i;
        ...
        if (i < code.length) {
            ...
            return finishTabifier(out, bindID);
        }
    }
