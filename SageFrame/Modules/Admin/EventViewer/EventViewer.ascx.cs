...
namespace SageFrame.Modules.Admin.EventViewer
{
    public partial class EventViewer : BaseAdministrationUserControl
    {
        ...
        {
            IncludeCss("eventViwerManagement", "/Administrator/Templates/Default/css/EventViewerMangement.css");
            if (!Page.IsPostBack)
            {
                PopulateLogType();
                BindGrid();
                imgLogClear.Attributes.Add("onclick", "javascript:return ConfirmDialog(this,'Confirmation','" + GetSageMessage("EventViewer", "AreYouSureToClearLogs") + "')");
                imgLogDelete.Attributes.Add("onclick", "javascript:return ConfirmDialog(this,'Confirmation','" + GetSageMessage("EventViewer", "AreYouSureToDeleteLogs") + "')");                
            }
        }
