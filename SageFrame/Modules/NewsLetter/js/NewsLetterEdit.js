$(function () {
    ...
    var NewsLetterEdit = {
        ...
        init: function () {
            ...
            $("#Email").show();
            $("#PhoneMessage").hide();
            $("#chkIsAllSubscriber").on("change", function () {
        ...
        SendEmailForSubscriber: function () {
            var emailToAll = $("#chkIsAllSubscriber").prop("checked");
            var subscriberList = '';
            if (emailToAll) {
                subscriberList = '';
            }
            else {
                subscriberList = $("#txtEmailList").val()
            }
            var bodymsg = '';
            ...
            NewsLetterEdit.config.url = NewsLetterEdit.config.baseURL + NewsLetterEdit.config.method;
            NewsLetterEdit.config.data = JSON2.stringify({ subscriberList: subscriberList, Subject: $("#txtSubject").val(), BodyMsg: bodymsg, emailToAll: emailToAll, UserModuleID: NewsLetterEdit.config.UserModuleID, PortalID: NewsLetterEdit.config.PortalID });
        ...
        BindSubscriberEmailList: function (data) {
            ...
            if (data.d.length > 0) {
                ...
                emailhtml += '</table>';
                emailhtml += '<a href="#" id="btnEmailOK" class="sfBtn smlbtn-succ icon-tick">Ok</a>';
            }
            else {
