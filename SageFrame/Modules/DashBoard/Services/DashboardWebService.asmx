...
public class DashboardWebService : AuthenticateService
{
    ...
    [WebMethod]
    public int AddUpdateGroup(GroupInfo objGroupInfo)
    {
        if (IsPostAuthenticated(objGroupInfo.PortalID, objGroupInfo.UserModuleID, objGroupInfo.AddedBy, objGroupInfo.SageFrameSecuretoken))
        {
            DashboardController objDashboard = new DashboardController();
            return objDashboard.AddUpdateGroup(objGroupInfo);
        }
        else
        {
            return 0;
    ...
    [WebMethod]
    public void DeleteGroupByID(GroupInfo objGroupInfo)
    {
        if (IsPostAuthenticated(objGroupInfo.PortalID, objGroupInfo.UserModuleID, objGroupInfo.AddedBy, objGroupInfo.SageFrameSecuretoken))
        {
            DashboardController objDashboard = new DashboardController();
            objDashboard.DeleteGroupByID(objGroupInfo.GroupID);
        }
    }
    [WebMethod]
    public int AddUpdateGroupItem(GroupItemInfo objGroupItem)
    {
        if (IsPostAuthenticated(objGroupItem.PortalID, objGroupItem.UserModuleID, objGroupItem.AddedBy, objGroupItem.SageFrameSecuretoken))
        {
            DashboardController objDashboard = new DashboardController();
            return objDashboard.AddUpdateGroupItem(objGroupItem);
        }
        else
        {
            return 0;
    ...
    [WebMethod]
    public void UpdateGroupOrder(GroupItemInfo objGroupItem, string groupOrder)
    {
        if (IsPostAuthenticated(objGroupItem.PortalID, objGroupItem.UserModuleID, objGroupItem.AddedBy, objGroupItem.SageFrameSecuretoken))
        {
            DashboardController objDashboard = new DashboardController();
            objDashboard.UpdateGroupOrder(groupOrder);
        }
    }
    #endregion
}