...
function ChangeIDWithParent($html) {
    EasyLibrary.GenerateAndAppendID($html);
    $html.find("div[data-id]").each(function ($index, $ele) {
        ...
        var WebManagement = {
            ...
            UpdateComponentManually: function () {
                ...
                $.each(component, function (i, v) {
                    ...
                    var type = typeof v.type === "undefined" ? "" : v.type;
            ...
            CompoBindEvent: function () {
                $('.downloadcompo').off().on('click', function () {
                    ...
                    if (typeof ($compo) !== "undefined") {
                        ...
                        var version = compo.Version;
                        var success = WebManagement.UpdateComponent(compo.ComponentName, $compo, compo.ComponentID, $compo.category, $compo.icon, classes, dependencies, version);
                        if (success) {
            ...
            SaveWebData: function (isPreview) {
                ...
                $cloneDOM.find('.menuHeader .eb-menu li').find('a').each(function () {
                    var $this = $(this);
                ...
                var $activePage = $cloneDOM.find('.menuHeader .eb-menu li > a.active-page');
            ...
            UpdateComponent: function (componentName, $componentValue, UniversalComponentID, category, iconClass, classess, dependencies, version) {
                var componentValue = JSONStringify($componentValue);
                var success = false;
                $.ajax({
                    isPostBack: false,
...
function FontSetting($parent,$textChange, properties) {
    InitEvents();
    LoadSettings();
    function InitEvents() {
        TextTranformCheck();
    ...
    function FontWidth() {
        ...
        if ($parentWidthClass !== null) {
            ...
            $(properties.WidthSlider).show();

        }
        else {
            $(properties.WidthSlider).hide();
        }
        ...
        AdvanceSageSlider($(properties.WidthSlider), $(properties.WidthHandle), 1, 100, fontWidth, fonstWidthSlider, $parent, '%');
    }
    function TextTranformCheck() {
        ...
        $(properties.TextTransform).val(trasformValue);
        $(properties.TextTransform).off().on('change', function () {
    ...
    function LoadSettings() {
        ...
        function FontSize(space) {
            var handleFontSize = $(properties.FontSizeHandle);
            handleFontSize.parent().next().find('.changeFontSize').val(space);
            $textChange.css('font-size', space + 'px');
        }
        AdvanceSageSlider($(properties.FontSizeSlider), $(properties.FontSizeHandle), minFontSize, maxFontsize, fontsize, FontSize, $parent, 'px');
        var letteSpace = 0;
        var removeClass = '';
        ...
        AdvanceSageSlider($(properties.LetterSpacingSlider), $(properties.LetterSpacingHandle), -10, 10, letteSpace, LetteSpaceChange, $parent, 'px');
        loadColorPicker();
        LoadFontFamily();
    }
    function loadColorPicker() {
       $(properties.FontColor).css('background-color', $textChange.css('color'));
        var colorPickerOption = ColorPickerOption({
            renderCallback: function ($elm, toggled) {
                var objColor = RenderCallBackColor(this);
        ...
        $(properties.FontColor).colorPicker(colorPickerOption);

    }
    function LoadFontFamily() {
        $(properties.FontFamily).html(DOMFontAdvanceCollection());
        ...
        $(properties.FontFamily).val(defaultFontFamily);
        
        (defaultFontFamily);
        if (typeof (classesList) !== "undefined") {
            var weightClasses = classesList.match(/f-weight-[0-9]{0,3}/g);
            if (weightClasses !== null) {
                $(properties.FontWeight).val(weightClasses[0].replace('f-weight-', ''));
            }
        }
        $(properties.FontWeight).off().on('change', function () {
        ...
        $(properties.FontFamily).off().on('change', function () {
            var classList = $textChange.attr('class');
            if (typeof (classesList) !== "undefined") {
                var fontClass = classList.match(/ff-(\w+)/g);
                if (fontClass !== null) {
            ...
            fontWeight($(this).val());
            $(properties.FontWeight).trigger('change');
        });
        function fontWeight(fontName) {
            var fontDOM = DOMFontWeight(fontName);
            if (fontDOM.length > 0) {
                $(properties.FontWeight).html(fontDOM);
            }
        }
    }
}
