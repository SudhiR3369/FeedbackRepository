...
function ActiveMenu() {
    ...
    $('.editor-site-header').find('.pageName').each(function () {
        if ($(this).text().toLowerCase() === currentpageName.toLowerCase()) {
            ...
            $(this).parent().css({ 'background-color': '', 'border-color': '' });
        }
    });
}
...
function FullPagePopup(fullpageOption) {
    ...
    if (option.showheading) {
        fullpageHeader = DOMCreate('div', DOMCreate('span', option.heading, option.classes.title) + DOMCreate('span', '<i class="fa fa-times"></i>', ' f-right fullpage-close-model'), option.classes.header, 'fullpagePopup');
    }
    var body = DOMCreate('div', option.data, option.classes.body);
    var fullpagPopup = DOMCreate('div', fullpageHeader + body, option.classes.innerwrapper, '', ['style="width:' + option.width + ';height:' + option.height + '"']);
...
function GetSingleAPIResult(key) {
    if (typeof apiResultString !== "undefined" && typeof apiResultString[key] !== "undefined")
        return apiResultString[key].Result[0];
    else
        return null;
}
/*
call when we need to update the meta tag in the page
*/
